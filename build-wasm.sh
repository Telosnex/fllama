...
# Function to check if emcc is installed
check_emcc_installed() {
    if ! command -v emcc &> /dev/null; then
        cat << EOF
Error: emcc (Emscripten compiler) was not found in your PATH.

To resolve this issue, please ensure you have Emscripten installed and that your environment is properly configured:

1. If Emscripten is not installed, visit https://emscripten.org/docs/getting_started/downloads.html for installation instructions.

2. If Emscripten is installed, activate the environment for the current shell session using:
...
export EMCC_CFLAGS="-O3 -msimd128 -pthread -frtti -sMEMORY64=1 -s USE_PTHREADS=1 -s ASYNCIFY=1 -s PTHREAD_POOL_SIZE=4 -DNDEBUG -flto=full -s ALLOW_TABLE_GROWTH -s SHARED_MEMORY=1 -s EXPORT_ALL=1 -s EXPORT_ES6=1 -s MODULARIZE=1 -s INITIAL_MEMORY=800MB -s MAXIMUM_MEMORY=4GB -s ALLOW_MEMORY_GROWTH -s FORCE_FILESYSTEM=1 -s EXPORTED_RUNTIME_METHODS=['addFunction'] -s EXPORTED_FUNCTIONS=['_fllama_get_bos_token_export, _fllama_get_eos_token_export, _fllama_cancel_inference_export, _fllama_tokenize_export, _fllama_get_chat_template_export, _fllama_inference_export, _malloc'] -s NO_EXIT_RUNTIME=1"
